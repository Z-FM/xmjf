...
function initTrHtml(investList) {
    if (null!=investList&&investList.length>0){
        var trs="";
        for (var i=0;i<investList.length;i++){
            var tempData=investList[i];
            trs=trs+"<tr>";
            //年化率
            trs=trs+"<td>" +
                "<strong>" +tempData.itemRate+
                "</strong>" +
                "<span>%" ;
            if (null!=tempData.itemAddRate&&tempData.itemAddRate!=""){
            ...
            if(tempData.total<=90&&tempData.total>85){
                trs=trs+"A";
            }
...
function initInvestScale() {
    ...
    $(".itemScale").each(function () {
        var investScale = $(this).attr("data-value");
